...
| Latest-only clean contract — strict versioned persistence, one viewer loader, no compatibility layer | **[BUILT — introduced at `engine-v0.11.0`; current release candidate `engine-v0.11.3`]** the line supports exactly ONE current contract: `MachineSnapshot` `schema_version="v0.11"` with REQUIRED typed goal/run_context and per-representation wire validation; observation bundle meta v2 (closed vocabulary, segment identity, one digest authority) written and read through `ObservationBundleMetaV2`/`load_bundle_meta_v2`; wait records `record_schema_version="wait-v1"`; the viewer parses metadata through the ONE engine loader with typed `node_status` as the only terminal authority; old persisted data is REJECTED naming its historical tag (no importers/adapters/dual readers — historical lines stay inspectable at their own tags); the old-wheel differential is replaced by a sealed current-contract oracle + machine-readable intentional-break ledger (transition closeout: 0 behavior deltas, 68/68 public deltas ledgered). Candidate matrix: engine `0.11.3`, tools `0.5.1`, viewer `0.3.1`, pins `>=0.11,<0.12`; current immutable release remains `engine-v0.11.2` until the candidate is cut. | Consumer canaries remain product-owned |
...
| Explicit node-context binding + native Codex console vision | **[BUILT — `engine-v0.11.3` / tools `0.5.1` release candidate]** every declared capability node consumes plan/machine/memory/model-profile settings through the bound door; fanout binds every item; subworkflow propagates plan/machine and rejects capability-only settings. Staged Codex images use native `--image`; Claude keeps path-scoped Read. Sealed behavior corpus remains zero-delta. | Tag, wheel hashes, paid qualification, and consumer canaries remain release/adoption gates. |
