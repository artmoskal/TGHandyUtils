...
| Latest-only clean contract — strict versioned persistence, one viewer loader, no compatibility layer | **[CANDIDATE — `engine-v0.11.0` pending tag; do not re-pin until cut]** the line supports exactly ONE current contract: `MachineSnapshot` `schema_version="v0.11"` with REQUIRED typed goal/run_context and per-representation wire validation; observation bundle meta v2 (closed vocabulary, segment identity, one digest authority) written and read through `ObservationBundleMetaV2`/`load_bundle_meta_v2`; wait records `record_schema_version="wait-v1"`; the viewer parses metadata through the ONE engine loader with typed `node_status` as the only terminal authority; old persisted data is REJECTED naming its historical tag (no importers/adapters/dual readers — historical lines stay inspectable at their own tags); the old-wheel differential is replaced by a sealed current-contract oracle + machine-readable intentional-break ledger (transition closeout: 0 behavior deltas, 68/68 public deltas ledgered). engine `0.11.0`, tools `0.5.0`, viewer `0.3.0`, pins `>=0.11,<0.12` | Release gate: cumulative qualification + independent review + user-approved paid run/tag |
