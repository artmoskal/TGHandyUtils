...
| Latest-only clean contract — strict versioned persistence, one viewer loader, no compatibility layer | **[BUILT — introduced at `engine-v0.11.0`; candidate `engine-v0.11.6`, tagged current `engine-v0.11.5`]** the line supports exactly ONE current contract: `MachineSnapshot` `schema_version="v0.11"` with REQUIRED typed goal/run_context and per-representation wire validation; observation bundle meta v2 (closed vocabulary, segment identity, one digest authority) written and read through `ObservationBundleMetaV2`/`load_bundle_meta_v2`; the v0.11.6 candidate advances wait records from v0.11.5's `wait-v1` to `record_schema_version="wait-v2"` with REQUIRED persisted registration-attempt identity; the viewer parses metadata through the ONE engine loader with typed `node_status` as the only terminal authority; old persisted data is REJECTED naming its historical tag (no importers/adapters/dual readers — historical lines stay inspectable at their own tags); the old-wheel differential is replaced by a sealed current-contract oracle + machine-readable intentional-break ledger (transition closeout: 0 behavior deltas, 68/68 public deltas ledgered). Candidate matrix: engine `0.11.6`, tools `0.5.1`, viewer `0.3.1`, pins `>=0.11,<0.12`. | Consumer canaries remain product-owned |
