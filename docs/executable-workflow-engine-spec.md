...
(pure derived-state reducer, byte-free LOUD validation, weak-model anti-repeat proven by test),
`WindowedMemory` (last-N verbatim + findings-preserving dropped notice), `CompactingMemory`
(threshold-gated rule-based compaction, pluggable compactor, no hidden model calls), plus
...
| Workflow memory T2 prompt policies | **[BUILT v0.8.0]** `StructuredStateMemory` + `WindowedMemory` + `CompactingMemory` + per-node `memory=` selection (GoPro P1 named-consumer request; AC-S1..S6 test-locked) | Byte-free LOUD validation; memory stays input-never-control; compactor is code — LLM compactors are product-injected, never hidden model calls |
...
| Workflow memory T2 prompt policies (§0/§12) | **[BUILT]** — v0.8.0: structured_state / windowed / compacting + per-node `memory=` config |
