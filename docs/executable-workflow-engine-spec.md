...
**`FlowArtifact` v1.5a — authored fanout + turnkey author. [SHIPPED — tag `engine-v0.9.0`]** The delivered SLICE of
...
3. **`WorkflowExecutor`** — coordinates a `WorkflowDefinition` + `RuntimeProfile` end-to-end through
   focused internal owners for compilation, node services/scheduling, suspension, result assembly,
   observation writing, and retention. It remains the one public machine runtime while product
   behavior stays in declared workflows and registered capabilities.
...
This numbered list is the evidence-registry boundary for binding engine promises. Other sections
explain architecture and scope; adding a new guarantee here requires a named behavioral test (and,
where relevant, sealed-corpus or state-algebra evidence) in
`tests/fixtures/promise_evidence_registry.json`.

1. **Universal invocation door.** Every capability execution, including each fan-out item, crosses
   the engine-owned capability runtime and its policy, budget, timeout, usage, and observation
   mechanics. Inspection may read the registry; execution may not call a registered handler around
   that door.
2. **Cumulative accepted state.** Retry, retrace, replan, resume, and nested execution preserve
   accepted sibling outputs, evidence, artifacts, usage, and terminal partial truth unless the
   declared state-algebra rule explicitly replaces that same key.
3. **Explicit node-context binding.** A node kind consumes each configured plan card, machine card,
   memory policy, and model profile at its real invocation boundary or graph validation rejects the
   unsupported field. Fan-out binds every item; subworkflow binds parent plan/machine metadata and
   rejects capability-only memory/model settings.
4. **Complexity gradient.** A one-step consumer uses the same executor and capability runtime as a
   durable, authored, retraced workflow without loading or configuring unrelated optional
   subsystems. Product semantics remain capabilities/packs rather than product branches in the
   executor.
...
| Durable waits + observation segments + related-run identity (§12b) | **[BUILT — introduced in `engine-v0.9.0`; current release `engine-v0.11.10` retains v0.11.6 handle-bound delivery and closes commit-to-exposure races]** engine owns wait identity/claim(CAS+lease)/bounded attempts/terminalization/trace; products own storage (`WaitCoordinator` + conformance kit), outbox delivery, and ALL clocks (`due()`/`stalled()`/cancel loop); append-only segment markers with one canonical attempt per logical index; optional `correlation_id` (Related-run ID) never controls execution; paid 4-consumer qualification 4/4 | R4-B opt-in suspended-group eviction shipped; parent-edge metadata for external trace activity is a post-v0.9 follow-up |
| `FlowArtifact` v1.5a (authorable bounded fanout + turnkey author factory) | **[BUILT — introduced in `engine-v0.9.0`; current release `engine-v0.11.10`]** §6 — max_items REQUIRED, reject-never-clamp, two-engine handoff test-locked | v1.5 remainder below |
| Runtime truth contracts — enforced execution window, `partial` timeouts/planner tasks, interruptibility, retrace provenance | **[BUILT — shipped at `engine-v0.10.0`]** engine-owned resolver intersects task/capability/run/parent bounds (soft/hard/reserve, named limiting sources + clamps); a cooperatively timed-out capability is truthful `partial`, cancellation suppression is failed containment, planner `partial` stays terminal (never rewritten to `done`), and `RuntimeLimits.timeout_s` is ENFORCED; `CapabilitySpec.timeout_enforcement` (process/cooperative/none) — inline-sync under any finite window is rejected pre-handler, `external`≠process; CLI/console/external-process doors inherit one soft/hard window with represented terminate + reap/result-settlement time; the shared process owner bounds stdin and stops the spawned process group; graph hangs have a separate recorded cancellation allowance; typed `RetraceProvenance` reaches the target capability; viewer projects persisted window/process/timeout/retrace truth. Breaking changes recorded at the historical tag (the latest-only line ships no migration guides); tools `0.4.0`, viewer `0.2.2` | Consumer canaries remain product-owned |
| Bounded process I/O — result-settlement safety (§ this release) | **[BUILT — shipped at `engine-v0.10.1`]** `engine/process_io.py` owns bounded stream capture (O(cap) retained head+tail, truthful total/truncation) and race-resistant result-file reading (lstat+fstat regular-file only, O_NOFOLLOW/O_NONBLOCK, cap+1 bounded read) — FIFO/socket/device/symlink/oversize are rejected without reading, so a result path can no longer hang the loop, disclose a symlink target, or exhaust memory after the deadline; one settlement classifier (unsafe/oversize -> failed, timeout -> partial, no-result truncated-stdout -> partial); every process door inherits it; wait conformance gains due()/health(); viewer summarizes + caps detail bodies. Breaking: unsafe/oversized declared result is now failed. tools `0.4.1`, viewer `0.2.3` | Consumer canaries remain product-owned |
| Latest-only clean contract — strict versioned persistence, one viewer loader, no compatibility layer | **[BUILT — introduced at `engine-v0.11.0`; v4 source candidate under qualification]** the line supports exactly ONE current contract: `MachineSnapshot` `schema_version="v0.11"` with REQUIRED typed goal/run_context and per-representation wire validation; observation bundle v4 has closed segment metadata, one logical detail body, SHA-256 identity over canonical uncompressed bytes, small inline bodies, and large run-scoped values read through `ObservationReader`; trace/detail-envelope/usage finalization is incremental. The viewer consumes the ONE strict v4 loader and compact reader with typed `node_status` as terminal authority; old persisted data is rejected with no importer, adapter, or dual reader. The published `engine-v0.11.19` line remains the historical v3 reader until a separately gated v4 release. The sealed current-contract oracle and intentional-break ledger govern the transition. | Consumer canaries remain product-owned |
| Runtime ownership decomposition — focused executor, bundle, and viewer owners | **[BUILT — shipped at `engine-v0.11.1`]** `WorkflowExecutor` coordinates dedicated compiler, node/scheduling, suspension, and result-assembly owners; observation contract/writer/retention are one-way; viewer storage/grouping/projection/rendering/transport are separate truth layers. Internal modules are not consumer extension points. Sealed current-contract corpus reports zero behavior/public-surface delta; four consumer-shaped hermetic workflows and installed-wheel viewer smoke stay green. | Future universal mechanics land in the focused owner or a capability/pack, never as product branches in the executor. |
| Planner retrace output accumulation | **[BUILT — corrected at `engine-v0.11.2`]** one ordered merge rule preserves earlier-round task outputs, lets current execution deterministically replace colliding keys, and emits no `output_ref` for successful no-output tasks; public two-round retrace, trace, usage, and persisted-bundle evidence are test-locked. | MageQA removes its downstream patch only after its consumer canary passes against the unmodified wheel. |
| Explicit node-context binding + native Codex console vision | **[BUILT — released at `engine-v0.11.3` / tools `0.5.1`]** every declared capability node consumes plan/machine/memory/model-profile settings through the bound door; fanout binds every item; subworkflow propagates plan/machine and rejects capability-only settings. Staged Codex images use native `--image`; Claude keeps path-scoped Read. A live two-image semantic canary passed with one recorded engine usage event; the sealed behavior corpus remains zero-delta. | Consumer canaries remain adoption gates. |
...
## 12b. DURABLE WAITS + OBSERVATION SEGMENTS (SHIPPED — tag `engine-v0.9.0`)

Status: shipped in **engine-v0.9.0** (independently reviewed; paid cross-consumer
qualification 4/4 + user viewer acceptance). Migration is a breaking-but-mechanical
...
**Doors.** Durable suspension returns a complete `WaitHandle` ONLY (no snapshot); products
persist that handle, including its opaque `registration_id`. The one continuation door is
`await engine.deliver_wait_event(handle, event)` → typed
...
failed evidence write is visible and repaired by any redelivery). `wait_id` is a deterministic
...
**Retention (user-settled policy).** Suspended (in-flight) logical runs are never retention
candidates. Their run-scoped value store is required to preserve complete grouped evidence on a
later resume, so deleting observation history independently is not a truthful viewer-only action.
Finalized groups follow the configured retention limit. Durable snapshots remain coordinator-owned;
local waits still require the caller to retain the snapshot.
