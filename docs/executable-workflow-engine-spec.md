...
| Latest-only clean contract — strict versioned persistence, one viewer loader, no compatibility layer | **[BUILT — introduced at `engine-v0.11.0`; current release `engine-v0.11.10`]** the line supports exactly ONE current contract: `MachineSnapshot` `schema_version="v0.11"` with REQUIRED typed goal/run_context and per-representation wire validation; observation bundle meta v3 (closed vocabulary, segment identity, one digest authority, required provider-evidence integrity) written and read through `ObservationBundleMetaV3`/`load_bundle_meta_v3`; v0.11.6 advances wait records from v0.11.5's `wait-v1` to `record_schema_version="wait-v2"` with REQUIRED persisted registration-attempt identity; the viewer parses metadata through the ONE engine loader with typed `node_status` as the only terminal authority; old persisted data is REJECTED naming its historical tag (no importers/adapters/dual readers — v2 stays inspectable at `engine-v0.11.9`); the old-wheel differential is replaced by a sealed current-contract oracle + machine-readable intentional-break ledger. Current matrix: engine `0.11.10`, tools `0.6.1`, viewer `0.3.3`, pins `>=0.11,<0.12`. | Consumer canaries remain product-owned |
