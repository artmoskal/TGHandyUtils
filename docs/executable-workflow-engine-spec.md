...
| Durable waits + observation segments + related-run identity (§12b) | **[BUILT — introduced in `engine-v0.9.0`; supported adoption target `engine-v0.9.2`]** engine owns wait identity/claim(CAS+lease)/bounded attempts/terminalization/trace; products own storage (`WaitCoordinator` + conformance kit), outbox delivery, and ALL clocks (`due()`/`stalled()`/cancel loop); append-only segment markers with one canonical attempt per logical index; optional `correlation_id` (Related-run ID) never controls execution; paid 4-consumer qualification 4/4 | R4-B opt-in suspended-group eviction shipped; parent-edge metadata for external trace activity is a post-v0.9 follow-up |
| `FlowArtifact` v1.5a (authorable bounded fanout + turnkey author factory) | **[BUILT — introduced in `engine-v0.9.0`; supported adoption target `engine-v0.9.2`]** §6 — max_items REQUIRED, reject-never-clamp, two-engine handoff test-locked | v1.5 remainder below |
| Runtime truth contracts — enforced execution window, `partial` timeouts/planner tasks, interruptibility, retrace provenance | **[RELEASE CANDIDATE — targets `engine-v0.10.0`; tag pending final independent review, do not pin yet]** engine-owned resolver intersects task/capability/run/parent bounds (soft/hard/reserve, named limiting sources + clamps); a timed-out capability is a truthful `partial` (not laundered `failed`), planner `partial` stays terminal (never rewritten to `done`), `RuntimeLimits.timeout_s` is now ENFORCED; `CapabilitySpec.timeout_enforcement` (process/cooperative/none) — inline-sync with a declared finite window is rejected pre-handler, `external`≠process; CLI agents inherit the engine window (no hidden 600s); typed `RetraceProvenance` delivered to the retraced capability; viewer projects window/timeout/retrace from persisted truth. Breaking changes in `migration-v0.10.md`; tools `0.4.0`, viewer `0.2.2` | MageQA E0 adoption gate; post-tag consumer canaries |
