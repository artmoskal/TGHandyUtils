...
(pure derived-state reducer, byte-free LOUD validation hardened in v0.8.1, weak-model anti-repeat proven by test),
`WindowedMemory` (last-N verbatim + findings-preserving dropped notice), `CompactingMemory`
...
| Workflow memory T2 prompt policies | **[BUILT v0.8.0; HARDENED v0.8.1]** `StructuredStateMemory` + `WindowedMemory` + `CompactingMemory` + per-node `memory=` selection (GoPro P1 named-consumer request; AC-S1..S6 test-locked) | Byte-free LOUD validation traverses Pydantic/dataclass/mapping/sequence AND arbitrary-object attributes (`__dict__`/`__slots__`) to catch nested media/bytes, plus rendered data-URI rejection; attribute-less value objects (datetime/UUID/Decimal/numpy scalars) pass, opaque default-repr objects (`object()`/empty-slots) fail loudly — no false positives; memory stays input-never-control; compactor is code; unsafe `compacting(inner=structured_state)` fails loudly — use `structured_state(base=compacting)` |
