...
| AC-24 | Explicit `[i langvoice source->pt gen]` requests create one visual language card with target-language answer text and generated pronunciation audio attached to the Anki back side. | `services/content/anki_directives.py`, `services/content/anki_generation_graph.py`, `packages/ai_workflow_tools/ai_workflow_tools/media/voice_generation.py`, language voice graph and processor delivery tests. | Validated with mocked providers; paid live provider run deferred. |
