...
RULES:
- Each card tests ONE fact or idea. Prefer several focused cards over one broad card.
...
- If the content is too thin for a meaningful card, return a single card that captures its gist.
...
"""
    ...
    def extract_cards(self, content: str) -> List[AnkiCard]:
        ...
        try:
            prompt_text = self._prompt.format(content=content)
