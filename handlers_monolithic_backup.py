"""Unified recipient system handlers."""
...
async def handle_task_creation_response(message: Message, success: bool, feedback: Optional[str], actions: Optional[Dict[str, List[Dict[str, str]]]]):
    ...
    if not success:
        await message.reply("❌ Error creating task. Please try again.")
        return
...
async def process_user_input_with_photo(text: str, user_id: int, message_obj: Message, state: FSMContext, bot: Bot) -> bool:
    ...
    try:
        ...
        if message_obj.photo:
            ...
            await message_obj.reply("❌ No image found to process.")
            return False
        ...
        logger.error(f"🔍 DEBUG: Photo processing - combined content: {combined_content}")
...
async def process_thread_with_photos(message: Message, thread_content: List[Tuple], 
    ...
    try:
        ...
        await message.reply("❌ Error creating task from messages. Please try again.")
...
async def process_thread(message: Message, thread_content: List[Tuple[str, str]], 
    ...
    try:
        ...
        logger.error(f"🔍 DEBUG: Processing thread with {len(thread_content)} messages")
        logger.error(f"🔍 DEBUG: Concatenated content: {concatenated_content}")
...
async def handle_platform_type_selection(callback_query: CallbackQuery, state: FSMContext):
    ...
    logger.error(f"🔍 PLATFORM TYPE SELECTION: user={callback_query.from_user.id}, platform={platform_type}, current_mode={current_mode}, state_data={state_data}")
    ...
    try:
        ...
        if mode == "user_platform":
            ...
            next_actions_keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📝 Create Task Now", callback_data="create_task")],
                [InlineKeyboardButton(text="📱 Manage Recipients", callback_data="show_recipients")],
                [InlineKeyboardButton(text="⚙️ Settings", callback_data="show_settings")]
            ])
            
            await message.reply(
                f"✅ Successfully added shared recipient '{name}'!\n\n"
                f"🎯 Tasks created with this recipient will appear on their account.\n\n"
        ...
        await message.reply("❌ Failed to add account. Please check your credentials and try again.")
        await state.clear()
...
async def handle_trello_configuration(message: Message, state: FSMContext):
    ...
    try:
        ...
        if not boards:
            await message.reply(
                "❌ Unable to fetch boards. Please check your credentials and try again.\n\n"
...
async def handle_trello_board_selection(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback_query.answer("❌ Failed to load lists")
...
async def handle_trello_list_selection(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await state.clear()
        await callback_query.answer("✅ Trello configured successfully!")
...
async def back_to_trello_boards(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not boards:
            await callback_query.answer("❌ Unable to fetch boards")
            return
...
async def handle_recipient_name(message: Message, state: FSMContext):
    ...
    if not name:
        await message.reply("❌ Name cannot be empty. Please enter a name:")
        return
...
async def handle_toggle_recipient(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if success:
            await callback_query.answer("✅ Recipient status toggled!")
            # Refresh the edit view
            await edit_recipient(callback_query, state, recipient_id)
        else:
            await callback_query.answer("❌ Failed to toggle recipient status")
...
async def configure_recipient(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if recipient.platform_type == "trello":
            ...
            if not credentials:
                await callback_query.answer("❌ No credentials found for this recipient")
                return
        ...
        await callback_query.answer("❌ Error starting configuration")
...
async def configure_existing_trello(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not boards:
            await callback_query.message.edit_text(
                "❌ Unable to fetch boards. Please check if your Trello credentials are still valid.\n\n"
        ...
        await callback_query.message.edit_text(
            "❌ Failed to load Trello boards. Please check your credentials.\n\n"
...
async def remove_recipient(callback_query: CallbackQuery, state: FSMContext, recipient_id: str):
    ...
    try:
        ...
        if success:
            await callback_query.answer("✅ Recipient removed!")
            ...
            if recipients:
                ...
                for recipient in recipients:
                    status = "✅" if recipient.enabled else "❌"
            ...
            await callback_query.answer("❌ Failed to remove recipient.")
...
def get_platform_help_text(platform_type: str) -> str:
    ...
    if platform_type == "todoist":
        return ("📝 TODOIST SETUP GUIDE\n\n"
                ...
                "4. Paste it below\n\n"
                "💡 The token looks like: 0123456789abcdef...\n\n"
                "✏️ Enter your Todoist API token:")
        ...
        return ("📋 TRELLO SETUP GUIDE\n\n"
                ...
                "💡 Format example: abc123:def456789...\n\n"
                "✏️ Enter your credentials (API_KEY:TOKEN):")
...
async def confirm_transcription(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback_query.answer("❌ Error processing voice message")
        await state.clear()
...
async def cancel_transcription(callback_query: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback_query.message.edit_text("❌ Voice message transcription cancelled.")
    await callback_query.answer()
...
async def handle_task_creation(message: Message, state: FSMContext):
    ...
    if not task_description:
        await message.reply("❌ Task description cannot be empty. Please enter a description:")
        return
...
async def handle_owner_name_input(message: Message, state: FSMContext):
    ...
    if not owner_name:
        await message.reply("❌ Name cannot be empty. Please enter your name:")
        return
    ...
    try:
        ...
        if success:
            ...
            await message.reply("❌ Failed to update name. Please try again.")
        
        await state.clear()
        
    except Exception as e:
        logger.error(f"Failed to update owner name for user {user_id}: {e}")
        await message.reply("❌ Error updating name. Please try again.")
        await state.clear()
...
async def handle_location_input(message: Message, state: FSMContext):
    ...
    if not location:
        await message.reply("❌ Location cannot be empty. Please enter your location:")
        return
    ...
    try:
        ...
        if success:
            ...
            await message.reply("❌ Failed to update location. Please try again.")
        
        await state.clear()
        
    except Exception as e:
        logger.error(f"Failed to update location for user {user_id}: {e}")
        await message.reply("❌ Error updating location. Please try again.")
        await state.clear()
