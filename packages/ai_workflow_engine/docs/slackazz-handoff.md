...
Status: **ready for consumer validation against `engine-v0.9.2`**. SlackAzzCovered should pin tag
`engine-v0.9.2`, implement its Redis coordinator, and pass the engine conformance kit plus product
...
|---|---|
| `ai-workflow-engine==0.9.2` | Required workflow/wait runtime |
| `ai-workflow-tools==0.3.0` | Add for CLI agents/tool catalog if the product uses them |
| `ai-workflow-viewer==0.2.1` | Developer diagnostics or product-linked run inspection |
