...
Start a new empty observation root when adopting v0.11.15; inspect bundle v2 only with
`engine-v0.11.9`.
...
|---|---|
| `ai-workflow-engine==0.11.15` | Required workflow/wait runtime |
| `ai-workflow-tools==0.6.6` | Add for generic OpenAI-compatible clients, CLI agents, or tool catalog |
| `ai-workflow-viewer==0.3.8` | Developer diagnostics or product-linked run inspection |
...
(pin tag `engine-v0.11.15`, verify the published `release-manifest-v2` directory, then install the
