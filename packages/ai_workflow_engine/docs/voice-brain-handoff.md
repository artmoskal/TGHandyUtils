...
The product must pin tag `engine-v0.11.11`, verify the complete release directory, then run its own
...
- `ai-workflow-engine==0.11.11` for the runtime and custom `LLMCallable`.
- `ai-workflow-tools==0.6.2` for bounded non-streaming OpenAI-compatible calls or when using
...
- `ai-workflow-viewer==0.3.4` in diagnostics, not the real-time audio path.
