...
The product must pin tag `engine-v0.11.16`, verify the complete release directory, then run its own
...
- `ai-workflow-engine==0.11.16` for the runtime and custom `LLMCallable`.
- `ai-workflow-tools==0.6.7` for bounded non-streaming OpenAI-compatible calls or when using
...
- `ai-workflow-viewer==0.3.9` in diagnostics, not the real-time audio path.
