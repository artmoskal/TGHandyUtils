...
Consume it the way GoPro does — pin an **immutable tag** (`engine-v0.8.1`), build a wheel, and never
...
```
ai-workflow-engine @ git+ssh://git@<host>/<repo>@engine-v0.8.1#subdirectory=packages/ai_workflow_engine
ai-workflow-tools  @ git+ssh://git@<host>/<repo>@engine-v0.8.1#subdirectory=packages/ai_workflow_tools
```

(or build wheels from the tag via a detached worktree and vendor them, like GoPro's
`tools/build_engine_wheel.sh`). Current tags: `engine-v0.8.1` is latest (the Ask-1 passthrough
