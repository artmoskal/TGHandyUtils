...
## Release Artifacts And Verification (v0.11.13)
...
git tag -a "$TAG" \
  -m "$TAG" \
  -m "Source: $SOURCE" \
  -m "Matrix: ai-workflow-engine 0.11.13 / ai-workflow-tools 0.6.4 / ai-workflow-viewer 0.3.6" \
...
python3 "$TOOL" run-gate --name smoke --record "$OUT/smoke.json" \
  ...
  --wheel "$OUT/wheels-a/ai_workflow_engine-0.11.13-py3-none-any.whl" \
  --wheel "$OUT/wheels-a/ai_workflow_tools-0.6.4-py3-none-any.whl" \
  --wheel "$OUT/wheels-a/ai_workflow_viewer-0.3.6-py3-none-any.whl"
...
python3 "$TOOL" assemble --repo "$BUILD_A" --tag "$TAG" \
  ...
  --wheel "$OUT/wheels-a/ai_workflow_tools-0.6.4-py3-none-any.whl" \
  --wheel "$OUT/wheels-a/ai_workflow_viewer-0.3.6-py3-none-any.whl" \
  --second-wheel "$OUT/wheels-b/ai_workflow_engine-0.11.13-py3-none-any.whl" \
  --second-wheel "$OUT/wheels-b/ai_workflow_tools-0.6.4-py3-none-any.whl" \
...
```bash
BUNDLE=/path/to/downloaded/engine-v0.11.13
TRUSTED=/path/to/trusted/engine-v0.11.13-verifier
python3 "$TRUSTED/release_artifacts.py" verify-bundle --dir "$BUNDLE"
python -m pip install "$BUNDLE/ai_workflow_engine-0.11.13-py3-none-any.whl"
```
