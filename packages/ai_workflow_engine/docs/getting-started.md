...
The current immutable release is `engine-v0.11.12`. Do not install packages from a branch. Consume
the complete published directory, verify it with the tagged verifier as described in
[operations](operations.md#release-artifacts-and-verification-v01112), then record the exact bytes
installed:

```text
engine_tag=engine-v0.11.12
engine_source_commit=<git rev-parse engine-v0.11.12^{}>
