...
The candidate retains strict bundle v4 and its bounded engine-owned readers, and makes Codex CLI
failure diagnostics bounded and causal in the optional tools wheel. It does not change GoPro's
direct-VLM workflow contract; GoPro may continue consuming only the engine wheel.
Until the candidate is published, GoPro should pin tag `engine-v0.12.1`. After publication it must
verify the complete v0.12.2 release directory, record the source
...
`engine-v0.12.2` carries forward v0.12.1's bounded bulk reader, v0.12.0's bundle-v4 contract,
and v0.11.18's complete-child
execution breaker shared by
...
| Package | GoPro use |
|---|---|
| `ai-workflow-engine==0.12.2` | Runtime for complete-child execution windows and bundle-v4 writing/reading |
| `ai-workflow-tools==0.7.1` | Add only for provider clients, CLI agents, or media helpers |
| `ai-workflow-viewer==0.4.1` | Bundle-v4 diagnostic companion; not required in the detector image |
...
the engine `0.12.2` wheel for the current sidecar) and
