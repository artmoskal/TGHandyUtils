...
GoPro should pin tag `engine-v0.11.15`, verify the complete release directory, record the source
...
| Package | GoPro use |
|---|---|
| `ai-workflow-engine==0.11.15` | Required in the detection/sidecar runtime |
| `ai-workflow-tools==0.6.6` | Add for generic OpenAI-compatible clients, CLI agents, or media helpers |
| `ai-workflow-viewer==0.3.8` | Developer/diagnostic service; not required in the detector image |
...
(pin tag `engine-v0.11.15`, verify the published `release-manifest-v2` directory, then
install only the engine `0.11.15` wheel for the current sidecar) and
