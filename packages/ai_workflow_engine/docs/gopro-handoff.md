...
GoPro should pin tag `engine-v0.11.18`, verify the complete release directory, record the source
...
`engine-v0.11.18` adds one complete-child execution breaker shared by
...
| Package | GoPro use |
|---|---|
| `ai-workflow-engine==0.11.18` | Runtime for complete-child execution windows |
| `ai-workflow-tools==0.6.9` | Add only for provider clients, CLI agents, or media helpers |
| `ai-workflow-viewer==0.3.11` | Diagnostic companion; not required in the detector image |
...
(pin tag `engine-v0.11.18`, verify the published `release-manifest-v2` directory, then
install only the engine `0.11.18` wheel for the current sidecar) and
