...
`0.11.12 / 0.6.3 / 0.3.5` matrix after verifying its immutable release directory and passing E0.
...
wait records) with NO migration layer. MageQA must pin tag `engine-v0.11.12`,
...
`engine-v0.11.9` and start a new empty observation root for v0.11.12.
...
| Package | MageQA use |
|---|---|
| `ai-workflow-engine==0.11.12` | Required orchestration/runtime |
| `ai-workflow-tools==0.6.3` | Generic OpenAI-compatible clients, CLI agents, tool catalog, media helpers |
| `ai-workflow-viewer==0.3.5` | Low-level engine run investigation and bundle rendering |
...
(pin tag `engine-v0.11.12`, verify the published `release-manifest-v2` bundle, then
install engine `0.11.12`, tools `0.6.3`, and viewer `0.3.5`) and
