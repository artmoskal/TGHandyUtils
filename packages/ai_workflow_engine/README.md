...
> **`engine-v0.9.0` is the current release.** Pin the immutable tag and build a wheel. Never depend on a
...
git -C /tmp/tghandy-engine checkout --detach engine-v0.9.0
python -m pip wheel --no-deps -w ./vendor \
  /tmp/tghandy-engine/packages/ai_workflow_engine
python -m pip install ./vendor/ai_workflow_engine-0.9.0-py3-none-any.whl
python -c "import ai_workflow_engine as e; assert e.__version__ == '0.9.0'"
```
...
|---|---|
| `ai-workflow-engine==0.9.0` | Always. Core builder, executor, memory, waits, observation writer. |
